...
def main(msg, dict):
    ...
    for i in range(len(msg)):
        ...
        for j in range(len(splitted)):
            if(splitted[j].isalpha()):
                splitted[j] = reversed_dict[splitted[j]]
        msg[i] = "".join(splitted)
    for i in range(len(msg)):
        msg[i] = msg[i][::-1]
